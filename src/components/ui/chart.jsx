...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "border-border-color/50 bg-primary-bg text-primary-text grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl",
        className
      )}>
      ...
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn(
              "[&>svg]:text-secondary-text flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3"
            )}>
