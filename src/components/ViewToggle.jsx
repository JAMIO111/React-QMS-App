...
const ViewToggle = ({ viewGrid, setViewGrid }) => {
  return (
    <button
      ...
      className="relative flex flex-row justify-between items-center border border-border-color rounded-xl text-primary-text w-18 py-2 overflow-hidden cursor-pointer">
      {/* Sliding indicator */}
      <div
      ...
      <div className="flex w-full justify-between items-center relative z-10 px-2">
        <BsViewList
          className={`h-4 w-4 ${
            viewGrid ? "fill-primary-text" : "opacity-100 fill-primary-bg"
          }`}
        />
        <BsGrid
          className={`h-4 w-4 ${
            viewGrid ? "opacity-100 fill-primary-bg" : "fill-primary-text"
          }`}
        />
      </div>
    </button>
